...
\end{exe}
Furthermore, even the CC of properly factive predicates need not project, i.e., are not categorically presupposed to be true by the speaker. Consider {\em be aware} and {\em know}, which \citet{karttunen71b} took to be properly factive: the CC does not project in the naturally occurring examples in (\ref{nat2}) from \citealt{beaver-belly}.
...
The possibility of categorizing clause-embedding predicates by the projectivity of the CC is further called into question by the fact that the CC of all of the 20 predicates investigated is projective, albeit to varying degrees, compared to the main clause controls. {\bf JD, please describe Bayesian analysis and results here} In other words, speakers who utter an interrogative with one of the 20 clause-embedding predicates are more likely to be taken to be committed to the CC than to the main clause control content.\footnote{According to \citet[1739]{spector-egre2015}, it is not clear whether the CC of {\em say} is projective. Our Exp.~1 findings suggest that it is weakly projective.}    Thus, to distinguish factive predicates from optionally factive and non-factive ones in this set of 20 predicates, one would need to distinguish one group of projective CCs from another group of projective CCs. We propose that there is no principled way of doing so.

One might also wonder whether a different projection diagnostic might result in a categorical distinction between factive predicates, on the one hand, and optionally factive and non-factive ones, on the other. For instance, one might wonder whether a categorical distinctions emerges if binary certainty ratings are collected, that is, if participants are forced to identify whether or not the speaker is certain of the relevant content. To assess this possibility, we ran a follow-up experiment that was identical to the one reported on above, except that participants were asked to respond `yes' (coded as 1) or `no' (coded as 0) to the question of whether the speaker is certain of the relevant content (see Appendix \ref{a-binary} for details). Figure \ref{f-projectivity2} plots the proportion of `yes' ratings (indicating projection) by predicate, again in increasing order from left to right, as well as the ratings for the main clause stimuli (again abbreviated `MC'). As shown, the critical finding of Exp.~1 was  replicated: the CCs of the factive predicates are not categorically more projective than the CCs of the optionally factive predicates; as before, the CCs of {\em acknowledge, hear} and {\em inform} are at least as projective as the CCs of {\em reveal} or {\em discover}.\footnote{The finding that the CC of all 20 clause-embedding predicates is projective was also replicated. Compared to the projectivity of the CC of {\em be right}, which were the least projective of all CCs, the CCs of the other 19 predicates were more projective. {\bf JD, please describe the model we fit here, and explain why we couldn't use the main clauses as reference level here (no variance).}} Thus, the findings of this version of the experiment also do not support the assumption that factive predicates are categorically distinguished from optionally factive and non-factive ones by the projectivity of the CC alone.
...
We cannot, of course, rule out the possibility that a different projection diagnostic may categorically distinguish factive predicates from optionally factive and non-factive ones. In the mean time, however, we conclude that projectivity alone does not distinguish factive predicates from optionally factive or non-factive ones. The experiments described in the next section were designed to explore whether the CC of the same 20 predicates is entailed, to identify whether entailment and projectivity jointly categorize clause-embedding predicates, as per the second definition of factive predicates.
...
\end{xlist}
\end{exe}
These two diagnostics for entailment were applied to the CCs of the 20 clause-embedding predicates in Exps.~2a and 2b, respectively. Both experiments included control stimuli where the two contents stand in an entailment relation, that is, for which the relevant inference definitely follows (Exp.~2a) and that are definitely contradictory (Exp.~2b). We assume that the CC of a clause-embedding predicate is not entailed if responses to the CC are significantly different from responses to these control stimuli and that the CC of a clause-embedding predicate is entailed if responses to the CC are not significantly different from responses to these control stimuli. Thus, identifying entailed CCs  requires arguing from a null result. We present the findings of the two experiments in sections \ref{s31} and \ref{s32}, and then discuss the findings jointly in section \ref{s33}: given the standard classification of the 20 clause-embedding predicates, we expect the CCs of factive and veridical non-factive predicates to be entailed.
...
In applying the inference diagnostic for entailment in Exp.~2a, we assumed that if the content of $\phi$ entails the content of $\psi$, then the truth of the content of $\psi$ definitely follows from true statements of $\phi$, that is, the strength of the inference is at ceiling. If the content of $\phi$ does not entail the content of $\psi$, then the strength of the inference is somewhere below ceiling. Exp.~2a investigated the strength of the inference from true statements of unembedded matrix sentences with the 20 clause-embedding predicates to the CC by collecting gradient inference ratings.
...
Figure \ref{f-veridicality-predicate} plots the mean inference ratings for the target stimuli by predicate, in increasing order from left to right, as well as for the non-entailing controls (abbreviated `non-ent.\ C') and the entailing controls (abbreviated `entailing C'). Factive predicates are given in purple, optionally factive predicates in orange, veridical non-factive predicates in blue, and non-veridical non-factive predicates in gray. Participants' individual ratings are given by light gray dots. As shown, the mean inference ratings for most factive predicates as well as the veridical non-factive predicate {\em be right} are quite high, suggesting strong inferences to the truth of the CC. However,  the mean inference ratings of the purportedly factive predicate {\em reveal} and of the purportedly veridical non-factive predicate {\em demonstrate} are lower, suggesting that the CCs of not all factive and veridical non-factive predicates we investigated is entailed. This conclusion is confirmed by the statistical analysis. {\bf JD, please describe the analysis and the findings} In other words, only the inference ratings to the CC of {\em prove} are statistically indistinguishable from those of the entailing controls, suggesting that the CCs of the remaining 19 predicates are not entailed.
...
Figure \ref{f-veridicality-predicate2} plots the mean contradictoriness ratings for the target stimuli by predicate, in increasing order from left to right, as well as for the non-contradictory controls (abbreviated `non-contrd.\ C')  and  the contradictory controls (`contradictory C'). Factive predicates are given in purple, optionally factive predicates in orange, veridical non-factive predicates in blue, and non-veridical non-factive predicates in gray. Participants' individual ratings are given by light gray dots. As shown, while the mean contradictoriness ratings of the veridical non-factive predicate {\em be right} is close to ceiling, those of the factive predicates and of the other veridical non-factive predicate are not, suggesting that the CCs of none of the factive and veridical non-factive predicates are entailed. This conclusion is confirmed by the statistical analysis. {\bf JD, please describe the analysis and the findings} In other words, none of the contradictoriness ratings for the CCs are statistically indistinguishable from those of the entailing controls, suggesting that the CCs of all 20 predicates are not entailed.
...
In Exps.~2, we applied two standard diagnostics for entailment to the CC of 20 clause-embedding predicates, with the goal of identifying predicates CC is entailed. To assess whether the CC of a particular predicate is entailed, we compared the inference ratings (Exp.~2a) and the contradictoriness ratings (Exp.~2b) for that CC to the ratings for control stimuli with entailed content: we assumed that the CC of a predicate is entailed if the ratings for that CC are statistically indistinguishable from these control stimuli, and not entailed if the ratings for that CC are significantly lower. Given these assumptions, we found that the CC of {\em prove} is entailed according to the inference diagnostic for entailment and that the CC of none of the 20 predicates is entailed according to the contradictoriness diagnostic for entailment. These findings suggest that the CCs of the 5 factive predicates we investigated, namely {\em know, see, discover, be annoyed} and {\em reveal}, as well as of the veridical non-factive predicate {\em demonstrate}, are not entailed.

One might ask, of course, whether there is a different way in which entailed CCs can be identified. For instance, one might consider all CCs entailed whose mean inference or contradictoriness ratings are at least as high as those of {\em demonstrate}, that is .85 or .72, respectively: this would mean that the CCs of the predicates typically taken to be factive or veridical non-factive are entailed, as well as the CCs of some optionally factive predicates. There is, however, no principled reason why the ratings for the CC of {\em demonstrate} should be the cut-off for entailment. It is also not principled to pick an arbitrary mean inference or mean contradictoriness rating, say .8, and to categorize CCs with a mean inference rating or mean contradictoriness rating of at least .8 as entailed, and CCs with mean ratings below .8 as not entailed. After all, one could just as well pick a threshold of .85 or .75, with the result that the CCs of different predicates would be considered entailed: for instance, on the contradictoriness diagnostic, the CC of {\em discover} (mean: .79) would not count as entailed with a threshold of .8, but would with one of .75, and the CC of {\em know} (mean: .84) would count as entailed with a threshold of .8, but not with one  of .85. 

One might also wonder whether a different entailment diagnostic might result in the CCs of more clause-embedding predicates being classified as entailed, for instance by forcing participants to identify in a two-alternative forced choice task whether the CC follows (on the inference diagnostic) or whether the speaker's utterance is contradictory (on the contradictoriness diagnostic). To assess this possibility, we ran experiments that were identical to Exps.~2, except that participants were asked to respond `yes' (coded as 1) or `no' (coded as 0) to the question of whether the CC follows or whether the speaker's utterance is contradictory (see Appendix \ref{a-binary} for details). The two plots in Figure \ref{f-binary} show the proportion of `yes' ratings on the two diagnostics by predicate, again in increasing order from left to right, as well as the ratings for two types of control stimuli. Jittered light gray dots indicate individual participants' ratings. As shown, these experiments replicated the findings of Exps.~2. On the inference diagnostic, the proportion of `yes' ratings are quite high for most factive predicates as well as the veridical non-factive predicate {\em be right}, but the proportion for the purportedly factive predicate {\em reveal} and the purportedly veridical non-factive predicate {\em demonstrate} are lower, suggesting again that the CCs of not all factive and veridical non-factive predicates we investigated are entailed. On the contradictoriness diagnostic, the proportion of `yes' ratings for {\em be right} are quite high, but those of the factive predicates and of the other veridical non-factive predicate are not. 

inference: Predicates for which there's no evidence that they differ from "prove": "be right", "discover", "know", "see".


contradictoriness: using "be right" (the second most entailing predicate) as reference level returns the output below. you can see that it considers all predicates to be less entailing than "be right"

 {\bf WHAT DO THE MODELS SAY}
...
by-participant variability: prosody, different predicate senses, prior
...
%\caption{Contradictoriness diagnostic for entailment}
%\end{subfigure}
%\caption{Properties of the CC of 20 English clause-embedding predicates: on the x-axis, the mean certainty rating (with 95\% confidence intervals) and, on the y-axis, the mean inference rating from Exp.~2a (panel a) and the mean contradictoriness rating from Exp.~2b (panel b). factive predicates are given in darker blue, veridical non-factive ones in lighter blue, non-veridical non-factive ones in brown and optionally factive ones in black.}\label{f-summary-gradient}
%
%\end{figure}
...
%
%\item \citealt[345]{vds92}: ``Note that [global accommodation, that is, projection, JT\&JD] is what we would expect given the intuitive notion of presupposition as information taken for granted and note also that this explains the intuition that presuppositions [...] are entailed by their matrix sentence.''
%
...
\section{Experiments with binary response tasks}\label{a-binary}
...
% ZOIB model: CC of prove is not different from entailing controls, but CC of be_right is 
