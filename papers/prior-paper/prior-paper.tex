...
\jt{should the section header be experimentS?}
...
We tested whether listeners' prior beliefs modulate projection, a question that has started to be explored, albeit with conflicting results \cite{mahler2020, lorson2018}.The results reported here confirm \citepos{mahler2020} results and expand on them in several ways. First, while \citeNP{mahler2020} manipulated only the political party affiliation of the speaker, the manipulation in Exps.~1 and 2 relied on 20 distinct properties of the individuals denoted by the subjects of the 20 clauses (e.g., whether Julian is more likely to dance salsa if he is German or Cuban, or whether Zoe is more likely to have calculated the tip, if she is 5 years old or a math major). Thus, the result of Exps.~1 and 2 suggests a general effect of prior beliefs on projection.

Second, \citeNP{mahler2020} investigated the effect of prior beliefs on projection for factive predicates (exemplified by four predicates, including {\em know}) vs.\ non-factive predicates (exemplified by three, including {\em think}). The results of Exps.~1 and 2 confirm \citepos{mahler2020} result that prior probability influences the projection of the CC of factive and non-factive predicates, but also show that the effect holds for a cross-section of 20 clause-embedding predicates that include cognitive (e.g., {\em know}), emotive (e.g., {\em be annoyed}), communication (e.g., {\em announce}), and inferential predicates (e.g., {\em prove}). For a critical view on the factive/non-factive distinction see \citeA{tonhauser-degen-factive}. \jd{need to make clearer why this is a difference; the uninitiated reader may think "well, maybe mahler tested 40 factive and non-factive predicates! and what is there beyond factive and non-factive predicates anyway?"}.\jt{better?}  Finally, prior beliefs and projection were measured in a within-participant design in Exp.~1, in contrast to \citeposs{mahler2020} experiment, which only measured projection.\jt{she measured the prior in a ``norming study'' that she doesn't describe in the paper} The results indicate that projection is a systematic function of individual listeners' subjective prior beliefs in the truth of a content. This in turn suggests that at least some by-participant variability observed in previous projection experiments (see, e.g., \citeNP{tbd-variability,tonhauser-degen-factive}) may be due to participants assigning different prior probabilities to the contents under investigation. 
