...
The difference between the findings of Exps.~2 and 3 stand in the way of conclusively identifying which predicates are factive. This difference also suggests that the two diagnostics do not both diagnose entailment, contrary to assumption. Given how widely these diagnostics are used, a pressing question for future research is which diagnostic is better suited to diagnosing entailment.\footnote{We do not engage with this question in this paper for reasons of space but point to two observations that are relevant in addressing this question. First, the entailed controls received very high ratings across Exps.~2 and 3, suggesting that all four measures are able to diagnose entailed content. \citet[329]{demarneffe-etal2012} suggested that veridicality ratings are influenced by pragmatic reasoning. While we agree, it is unlikely that pragmatic reasoning would only influence participants' ratings on the target stimuli, to the exclusion of the control stimuli. Second, the inference and contradictoriness diagnostics yielded comparable ratings for the 20 predicates, as evidenced by the very high Spearman rank correlations of .954 (for the two gradient measures) and .934 (for the two categorical measures); these coefficients include the two types of controls. This observation suggests that, even though the ratings for the CCs of the 20 predicates were overall lower with the  contradictoriness measures than the inference measures, both may be measuring a similar underlying concept.} Regarding our quest for factive predicates, our findings point to a larger issue. Recall from section \ref{s2} that the CCs of more predicates than previously assumed are projective, i.e., presupposed. A consequence of this empirical finding is that relatively more importance falls to entailment in identifying factive predicates, under definition (\ref{def}b): of all the predicates with projective, i.e., presupposed, CCs, those whose CC is entailed are factive and those whose CC is not entailed are not factive. For the 20 predicates we investigated, all of whose CCs are projective, i.e., presupposed, this had the unfortunate consequence that the set of factive predicates is, under the inference diagnostic, the heterogeneous set consisting of {\em prove, be right, see, discover, confirm} and possibly {\em know} or, under the contradictoriness diagnostic, the empty set. In other words, because so many more predicates have presupposed CCs, whichever predicate has an entailed CC also has a good chance of being factive. We conclude that considering entailment in the identification of factive predicates does not have the desired effect and, hence, that the definition of factive predicates in (\ref{def}b) fails.
