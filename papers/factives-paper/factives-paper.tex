...
The findings of Exps.~1 suggest that projection does not categorically distinguish factive predicates from others. We observed, contrary to what is expected on definition (\ref{def}b), that the CCs of predicates standardly classified as factive are not categorically more projective than the CCs of all other predicates. 

One might object to this conclusion and argue that this expectation -- that the CCs of factive predicates are more projective than those of all other predicates -- is too strong. Specifically, given that the CCs of factive predicates are among the most projective in both Exps.~1a and 1b, one might argue that it suffices to show that assuming a class of factive predicates explains some of the variability observed in the projection of the CCs of clause-embedding predicates. To engage with this possibility, we fitted models to the certainty ratings of Exps.~1a and 1b that predict projection from a binary factivity classification of the 20 predicates (for this analysis, the factive predicates are the five predicates in (\ref{pred}a) and the non-factive predicates are the other 15). \jt{details on the models and where they can be found. say that these models show that factivity is a significant predictor. but how much variance is explained? less than in the models reported on in sections \ref{s-exp1a} and \ref{s-exp1b}, which predicted the ratings from the individual predicates}.
