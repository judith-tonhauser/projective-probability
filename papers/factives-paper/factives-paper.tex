...
The differences between the findings of our Exps.~2 and 3 stand in the way of conclusively identifying which predicates are factive. One issue is that the inference and the contradictoriness diagnostic deliver different results about the CCs of which predicates are entailed. Our findings suggest the two diagnostics do not both diagnose entailment, contrary to assumption. Given how widely these diagnostics are used, future research should investigate which diagnostic is better suited to diagnosing entailment. 
...
%The predicates with the highest mean veridicality rating (1.7) are {\em explain, notice} and {\em give} (e.g., {\em You might ask why Microsoft is punished by the loss of an Internet Explorer user, given that Internet Explorer can be downloaded free.}).
