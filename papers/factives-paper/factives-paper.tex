...
\end{xlist}
\end{exe}
The experiments in section \ref{s2} investigated the definition in (\ref{def}a): Assuming projection as a diagnostic for presupposed CCs, the results of these experiments as well as the meta-analyses of three existing datasets suggest that projection does not identify a class of factive predicates. The experiments in section \ref{s3} investigated the definition in (\ref{def}b): The evidence from our experiments and meta-analyses of existing datasets suggests that depending on how entailment is diagnosed, there either are no factive predicates or that the set of predicates identified as factive is heterogeneous with respect to projection, i.e., not a natural class. In sum, the results of our experiments do not provide empirical evidence for a coherent class of factive predicates under either of the two definitions in (\ref{def}).

Our main motivation in conducting these investigations was to understand which predicates analyses of the projection of the CC of clause-embedding predicates apply to, as these analyses are generally limited to the CCs of factive predicates. The explanandum is assumed to be that the CCs of factive predicates, but not those of optionally factive or non-factive ones, are presuppositions, which may project. On some analyses (e.g., \citealt{heim83,vds92}), factive predicates lexically specify that the CC is required to be entailed by or satisfied in the relevant context; optionally factive and non-factive predicates do not carry such a lexical specification. On other analyses (e.g., \citealt{abrusan2011,abrusan2016,romoli2015,best-question}), projection of the CC is derived from the CC being entailed content that is backgrounded; non-entailed CCs of optionally factive and non-factive predicates are again outside of the scope of these analyses. Given the results of our experiments, we are unfortunately no closer to answering the question of which predicates these analyses apply to.\footnote{To be clear: Contemporary analyses of the projection of the CCs of factive predicates can predict the non-projection of factive CCs in particular utterances. For instance, in \citealt{heim83} and \citealt{vds92}, the CC is locally accommodated when its projection would result in a contradictory or uninformative utterance. Although projection of the CC does not result in contradictory or uninformative utterances in the stimuli in our Exps.~1, one might assume, as suggested by an anonymous reviewer, a) that some participants enrich the context in which the polar question stimuli were presented with information that does result in a contradiction or uninformativity, and b) that participants are more likely to enrich the context with such information with some factive predicates than others. Under these assumptions, these analyses may be compatible with the results of our Exps.~1, namely that the projection of the CC of factive predicates is heterogeneous and not categorically higher than that of other predicates. Neither assumption, however, strikes us as independently motivated.} 

Where do we go from here? There are two broad ways forward. First, one might object to the conclusion we have drawn from our results on a number of grounds. For instance, one might consider one of the two definitions in (\ref{def}) appropriate, but not the diagnostics for presupposed and entailed content that we used. Or one might propose a different definition, and test novel diagnostics for factive predicates under that definition. Investigations that follow up on such objections constitute an important avenue of research for analyses that posit factive predicates. It is also possible to maintain one of the two definitions and our diagnostics, but object to our conclusion on the grounds that different types of systematic experimental or participant noise may have obscured the boundaries between existing underlying categories. We engage with these possibilities in section \ref{s41}. The second broad way forward is to give up on the assumption that there is a coherent class of factive predicates (see also \citealt{karttunen2016} and \citealt{djaerv-thesis} for such moves). In section \ref{s43}, we discuss what this move would mean for projection analyses.

\subsection{Possible objections to our conclusion}\label{s41}
...
It has been suggested to us, in personal communication and by a reviewer, that the observed gradience in projection may be compatible with a binary factivity category in combination with two assumptions: first, that predicates may be ambiguous between a factive lexical entry, on which the CC is presupposed and hence projects, and a non-factive lexical entry on which the CC is not presupposed (see \citealt[1736]{spector-egre2015} for such a proposal); and, second, that interpreters may be uncertain about which lexical entry a speaker intended in their utterance. Under such a view, a categorical notion of factivity is upheld, but the average effect of the uncertainty experienced by listeners about the use of any given predicates, which may be resolved preferentially one way or another through pragmatic factors, may lead to the result that certain predicates appear generally more factive than others on average. 

The suggestion, then, was that one way to test whether it is possible to maintain a binary factivity category with these two assumptions is to fit so-called finite mixture models to the data \citep{jacobs1991, richardson1997}. These models allow for representing the possibility that multiple components, i.e., multiple sub-datasets, make up an overall dataset. For instance, the data for each predicate from Exp.~1a could be analyzed with mixture models with respect to how many components (one, two, three, four, five, \ldots) best account for the data. To illustrate, consider Figure \ref{fig:mixtures}, which shows sample distributions of certainty ratings together with the optimal number of Gaussian components.\footnote{Models were fit using the \texttt{mclust} and \texttt{mixtools} packages \citep{benaglia2009, scrucca2016}  in R, which use expectation maximization to estimate optimal parameter values.} Given the aforementioned assumptions, one could attribute certainty ratings that are best captured with one Gaussian component with a mean of .4, as in panel (a), to a predicate that only has a non-factive lexical entry. Likewise, one could attribute certainty ratings that are best captured with one Gaussian component with a mean of .85, as in panel (b), to a predicate that only has a factive lexical entry.  Certainty ratings like those in panels (c) and (d), which are best captured by two Gaussian components, might then be attributed to predicates that have both a factive and a non-factive lexical entry, with differences between (c) and (d) being due to the predicates interacting differently with pragmatic factors that modulate projection. In short, the projection gradience observed in our Exp.~1a could be hypothesized to come about by virtue of one or two components being combined in various ways. 

%whether the data is best accounted for by one component (e.g., a factive one, or a non-factive one) or two components (e.g., both a factive and a non-factive one). Figure \ref{fig:mixtures} shows examples of possible distributions of projection ratings that all have a mean of 0.7 but are optimally characterized by different numbers of Gaussian components.\footnote{Models were fit using the \texttt{mclust} and \texttt{mixtools} packages \citep{benaglia2009, scrucca2016}  in R, which use expectation maximization (EM) to estimate optimal parameter values.} \jt{factive/projective OK? Fig relies on colors}
%
%
%\begin{figure}[h!]
%\begin{subfigure}{.33\textwidth}
%\includegraphics[width=\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/example-1component}
%\caption{One Gaussian component.}
%\label{fig:1comp}
%\end{subfigure}
%\begin{subfigure}{.33\textwidth}
%\includegraphics[width=\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/example-2components}
%\caption{Two Gaussian components.}
%\label{fig:2comp}
%\end{subfigure}
%\begin{subfigure}{.33\textwidth}
%\includegraphics[width=\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/example-4components-reveal}
%\caption{Four Gaussian components.}
%\label{fig:reveal}
%\end{subfigure}
%\caption{Different possible rating distributions for a predicate with mean = .7 with overlaid optimal number of Gaussian curves.}
%\label{fig:mixtures}
%\end{figure}
...
\includegraphics[width=\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/judith-panel-b}
\caption{One Gaussian component. Mean = .4}
\label{fig:1comp}
\end{subfigure}
\begin{subfigure}{.4\textwidth}
\includegraphics[width=\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/judith-panel-a}
...
\begin{subfigure}{.4\textwidth}
\includegraphics[width=\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/judith-panel-c}
\caption{Two Gaussian components. Mean = .6}
\label{fig:2comp}
\end{subfigure}
\begin{subfigure}{.4\textwidth}
\includegraphics[width=\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/judith-panel-d}
...
\begin{subfigure}{.4\textwidth}
\includegraphics[width=\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/example-3components-discover}
...
\label{fig:reveal}
\end{subfigure}
\caption{Different possible rating distributions with overlaid optimal number of Gaussian curves.}
\label{fig:mixtures}
\end{figure}

We caution against this approach on conceptual grounds. What mixture models can do is supply us with an answer to the question of how many components optimally characterize the data. Mixture models are not, however, a theory of factivity or projection, and, as such, they do not specify what the generative model underlying the components is. Thus, while mixture models can be used to test the predictions of theories of factivity or projection that provide us with such a generative model, we caution against their application in the absence of such a theory. And, in fact, we do not currently have such a theory because the account sketched above, on which predicates may have a factive lexical entry, a non-factive one, or both, does not lead us to expect projection data distributions, like those in panels (e) and (f) of Figure \ref{fig:mixtures}, that are best captured by three or four components, respectively, let alone distributions that are best captured by five or six components. Worse yet, in the absence of such a theory, we don't even know how to interpret data that are best captured by more than two components.\footnote{For simplicity, we introduced mixture models with Gaussian mixtures. A preliminary investigation of the data from Exp.~1a suggests that 15 of the 20 predicates are best characterized by 3-6 components; see Supplement \ref{a-gaussian}.  However, the assumption of normality made in Gaussian mixture models does not hold in the bounded slider ratings data of Exp.~1a. One would need to fit mixtures of Beta distributions, for which statistical inference about optimal cluster number is much more complex.} In short, we do not currently have a principled reason for applying mixture models to our data.
...
Figure \ref{fig:mixtures-exp1a} plots...

\begin{figure}[h!]
\centering

\begin{subfigure}{.45\textwidth}
\includegraphics[width=.8\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/pretend-gaussian}
\caption{{\em pretend}. XX components.}
\label{fig:1comp}
\end{subfigure}
\begin{subfigure}{.45\textwidth}
\includegraphics[width=.8\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/be-right-gaussian}
\caption{{\em be right}. XX components.}
\label{fig:1comp}
\end{subfigure}

\begin{subfigure}{.45\textwidth}
\includegraphics[width=.8\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/think-gaussian}
\caption{{\em think}. XX components.}
\label{fig:1comp}
\end{subfigure}
\begin{subfigure}{.45\textwidth}
\includegraphics[width=.8\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/suggest-gaussian}
\caption{{\em suggest}. XX components.}
\label{fig:1comp}
\end{subfigure}


\begin{subfigure}{.45\textwidth}
\includegraphics[width=.8\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/say-gaussian}
\caption{{\em say}. XX components.}
\label{fig:1comp}
\end{subfigure}
\begin{subfigure}{.45\textwidth}
\includegraphics[width=.8\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/prove-gaussian}
\caption{{\em prove}. XX components.}
\label{fig:1comp}
\end{subfigure}

\begin{subfigure}{.45\textwidth}
\includegraphics[width=.8\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/confirm-gaussian}
\caption{{\em confirm}. XX components.}
\label{fig:1comp}
\end{subfigure}
\begin{subfigure}{.45\textwidth}
\includegraphics[width=.8\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/establish-gaussian}
\caption{{\em establish}. XX components.}
\label{fig:1comp}
\end{subfigure}

\begin{subfigure}{.45\textwidth}
\includegraphics[width=.8\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/demonstrate-gaussian}
\caption{{\em demonstrate}. XX components.}
\label{fig:1comp}
\end{subfigure}
\begin{subfigure}{.45\textwidth}
\includegraphics[width=.8\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/announce-gaussian}
\caption{{\em announce}. XX components.}
\label{fig:1comp}
\end{subfigure}

\begin{subfigure}{.45\textwidth}
\includegraphics[width=.8\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/confess-gaussian}
\caption{{\em confess}. XX components.}
\label{fig:1comp}
\end{subfigure}
\begin{subfigure}{.45\textwidth}
\includegraphics[width=.8\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/admit-gaussian}
\caption{{\em admit}. XX components.}
\label{fig:1comp}
\end{subfigure}

\begin{subfigure}{.45\textwidth}
\includegraphics[width=.8\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/reveal-gaussian}
\caption{{\em reveal}. XX components.}
\label{fig:1comp}
\end{subfigure}
\begin{subfigure}{.45\textwidth}
\includegraphics[width=.8\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/acknowledge-gaussian}
\caption{{\em acknowledge}. XX components.}
\label{fig:1comp}
\end{subfigure}

\begin{subfigure}{.45\textwidth}
\includegraphics[width=.8\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/hear-gaussian}
\caption{{\em hear}. XX components.}
\label{fig:1comp}
\end{subfigure}
\begin{subfigure}{.45\textwidth}
\includegraphics[width=.8\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/discover-gaussian}
\caption{{\em admit}. XX components.}
\label{fig:1comp}
\end{subfigure}

\begin{subfigure}{.45\textwidth}
\includegraphics[width=.8\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/inform-gaussian}
\caption{{\em inform}. XX components.}
\label{fig:1comp}
\end{subfigure}
\begin{subfigure}{.45\textwidth}
\includegraphics[width=.8\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/see-gaussian}
\caption{{\em see}. XX components.}
\label{fig:1comp}
\end{subfigure}

\begin{subfigure}{.45\textwidth}
\includegraphics[width=.8\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/know-gaussian}
\caption{{\em know}. XX components.}
\label{fig:1comp}
\end{subfigure}
\begin{subfigure}{.45\textwidth}
\includegraphics[width=.8\textwidth]{../../results/5-projectivity-no-fact/graphs/mixtures/be-annoyed-gaussian}
\caption{{\em be annoyed}. XX components.}
\label{fig:1comp}
\end{subfigure}

\caption{Density of slider certainty ratings from Exp.~1a with overlaid optimal number of Gaussian curves.}
