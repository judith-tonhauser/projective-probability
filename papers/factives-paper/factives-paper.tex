...
The results of  Exps.~1a and 1b suggest that the definition of factive predicates in (\ref{def}a), according to which a predicate is factive if and only if its CC is presupposed, fails to identify a class of factive predicates. We next discuss converging evidence to this effect from three additional datasets: the CommitmentBank (\citealt*{demarneffe-etal-sub23}), the MegaVeridicality dataset (\citealt{white-rawlins-nels2018,white-etal2018b}) and the VerbVeridicality dataset (\citealt{ross-pavlick2019}).\footnote{We re-plotted the data, obtained at \url{https://github.com/mcdm/CommitmentBank}, \url{http://megaattitude.io} and \url{https://github.com/alexisjihyeross/verb_veridicality}, respectively. For the analysis files, see the GitHub repository mentioned in footnote \ref{f-github}.}
...
We see two ways forward: one is to reject the conclusion and maintain the assumption that there is a class of factive predicates whose CCs are distinct from those of optionally factive and non-factive predicates. One might reject the conclusion on either methodological grounds or conceptual grounds. For example, one might argue that the task itself introduced noise into the response behavior that is independent of the properties of particular predicates, or that participants had uncertainty about whether a particular predicate is factive. While this is certainly a possibility, it is unclear how these accounts could explain the very similar qualitative order of predicates across the measures reported in Exps.~1, 2, and 3a vs.~1, 2, and 3b as well as the CommitmentBank, MegaVerdicality, and VerbVeridicality datasets.

But one might also take the results at face value, reject the conclusion on conceptual grounds, and continue the search for an empirical diagnostic of factive predicates. This would require integrating certain assumptions into existing analyses of presuppositions. For instance, under analyses like those developed in \citealt{heim83} and \citealt{vds92}, the CCs of factive predicates need not project but may be locally accommodated when the default global accommodation would result in a contradiction, uninformativity or problems with binding. \jd{i think the talk of local and global accommodation will be confusing to the generalist audience -- is there a way to make this more digestible?} Although the default global accommodation is not overridden by the contexts in which the polar questions were presented to participants in our Exps.~1, one might assume  (as suggested by an anonymous reviewer) that some participants enrich the context with information that does license local accommodation and that participants are more likely to enrich the context with such information with some factive predicates than others. Under these assumptions, these analyses may be compatible with the results of our Exps.~1, namely that the projectivity of the CC of factive predicates is heterogeneous and not categorically higher than that of optionally factive predicates. Neither assumption, however, strikes us as independently motivated, so the burden of proof lies with those who would maintain analyses that rely on a class of factive predicates. 
