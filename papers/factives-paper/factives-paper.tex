...
It is also possible, however, that the fact that our experiments did not provide empirical support for a categorical factivity distinction has nothing to do with the definitions investigated or the diagnostics used. Our results are in fact compatible with the assumption that one of the two definitions in (\ref{def}) identifies a class of factive predicates and that the diagnostics we used in our experiments are the appropriate diagnostics to use. This is a possibility if we consider that our experiments may have introduced noise into the response behavior that is independent of the predicates and that masked a binary factivity category. There are multiple ways in which noise might have been introduced: i) the predicate did not occur by itself, but was combined with a particular subject noun phrase and complement clause; ii) the sentence containing the predicate was used in an utterance (by a named speaker) or presented as a true proposition; iii) we used a particular response task to collect ratings, providing participants with particular response options; and iv) different participants provided the ratings. Each of i) to iv) can introduce systematic noise, which, one could hypothesize created gradience even though each predicate is either factive or not, thereby obscuring a binary factivity category. In other words, we cannot conclude from the fact that our experiments did not provide empirical support for a categorical factivity distinction that there is no such distinction because the distinction may exist but be obscured by noise.

Our experiments were designed to mitigate several possible sources of systematic noise: i) the predicates occurred in the same sentence structures and were combined with the same complement clauses; ii) each predicate was presented in a sentence/utterance/proposition in the same context; iii) we compared ratings across different tasks (gradient and binary tasks gave same results); and iv) we collected judgments from a lot of participants. Regarding projection, we found systematicity in gradient orderings across gradient and categorical response task, different embeddings (negation, questions,...), different kinds of data (meta-analyses, constructed vs.\ naturally occurring). See also \citealt{degen-tonhauser-openmind}. Regarding entailment, we found systematicity across gradient and categorical response task, different kinds of data (meta-analyses), and different diagnostics. This is what makes us think that it is unlikely that there is a binary factivity category that is covered up by systematic noise. But it is still a possibility. For example, participants may have filled in further contextual information and may have done so more for factive predicates, thereby obscuring a categorical factivity distinction. In sum, while the systematicity we have found is strongly suggestive of underlying gradience, we cannot rule out that there is no class of factive predicates.
...
The assumption that there is a class of factive clause-embedding predicates has played a central role in linguistic theorizing on a broad range of topics (see section \ref{s1}). In this section, we discuss the implications of our findings for one of these topics, namely projection analyses.
