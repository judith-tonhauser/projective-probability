...
\subsubsection{Objection 3: Projection gradience may reflect uncertainty about which category a predicate belongs to.}\label{s-412}
...
One way to test whether such an account might hold may seem to fit so-called finite mixture models to the data \citep{jacobs1991, richardson1997}. Mixture models allow for representing the possibility that multiple components, i.e. multiple sub-datasets, make up an overall dataset. For instance, the data from Exp.~1a could be analyzed with mixture models for whether the data from a particular predicate is best accounted for by just one (projective or non-projective) component or two (one projective and one non-projective one). Figure \ref{fig:mixtures} shows examples of possible distributions of projection ratings that all have a mean of 0.7 but are optimally characterized by different numbers of Gaussian components.\footnote{Models were fit using the \texttt{mclust} and \texttt{mixtools} packages \citep{benaglia2009, scrucca2016}  in R, which use expectation maximization (EM) to estimate optimal parameter values.}
