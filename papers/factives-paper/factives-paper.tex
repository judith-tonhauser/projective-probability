...
\end{xlist}
\end{exe}
The observation that some factive predicates are merely semi-factive again gives rise to the question of whether projection distinguishes factive predicates from others. It is possible that, overall, factive predicates are more projective than others, even if some factives are semi-factive, but whether that is the case is an open, empirical question.\jd{Again, I'd argue that it resolves the question rather than raising it.}\jt{Better?}\jd{maybe. perhaps we can chat in person abt this?}\footnote{Contrary to what \citealt{karttunen71b} assumed, non-projection of the CC of semi-factive predicates is possible not just in polar questions and conditionals. For instance, the naturally occurring examples in (i) and ii) show that the CC of the semi-factive predicates {\em discover} and {\em realize} need not project when embedded under negation or a possibility modal, respectively.
