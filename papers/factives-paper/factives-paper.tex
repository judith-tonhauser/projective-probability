...
\jt{This discussion came from section 2.3. I think we should remove it entirely because it builds on the assumption that the five predicates in (13a) are factive, and we have not found any reason to assume that they are, so this is a step back.}
...
Where do we go from here? We see two broad ways forward. The first one is to maintain the notion of factivity and continue the search for a set of identifying diagnostics. While our results strongly suggest that projection and entailment do not identify a class of factive predicates, those results are contingent on how we operationalized projection and entailment, and how we measured them behaviorally. Thus, our results do not conclusively show that there is no class of factive predicates. In section \ref{s41}, we discuss what evidence for a class of factive predicates might look like. The second way forward is to give up on the assumption that there is a coherent class of factive predicates (see also \citealt{karttunen2016} and \citealt{djaerv-thesis} for such moves). In section \ref{s42}, we discuss what this move would mean for projection analyses, one of the areas in which factivity has played a central role in theorizing. 

\subsection{Continuing the search for a class of factive predicates}\label{s41}

There are different paths on which one can continue the search for a class of factive predicates. One is to revise the definition of factive predicates and develop and test novel diagnostics for their identification. Another option is to maintain one of the definitions in (\ref{def}), but to use different diagnostics for presupposed or entailed content. Empirical investigations of such diagnostics constitute an important avenue of research for those who would posit a class of factive predicates.

It is also possible, however, that the fact that our experiments did not provide empirical support for a categorical notion of factivity has nothing to do with the definitions investigated or the diagnostics used. Our results are compatible with the assumption that one of the two definitions in (\ref{def}) identifies a class of factive predicates and that the diagnostics we used in our experiments are the appropriate diagnostics to use if the experimental setup introduced noise into participants' response behavior that masked a binary factivity category. There are multiple ways in which such noise might have been introduced: i) our stimuli did not consist of the predicates alone, but of sentences in which the predicates were combined with particular subject noun phrases and complement clauses; ii) the sentences containing the predicates were used in an utterance (by a named speaker) or presented as a true proposition; iii) we used particular response tasks to collect ratings, providing participants with particular response options; and iv) human participants provided the ratings. Each of i) to iv) could have introduce systematic noise that, one could hypothesize, created gradience even though each predicate is either factive or not. %In other words, we cannot conclude from the fact that our experiments did not provide empirical support for a categorical factivity distinction that there is no such distinction because the distinction may exist but be obscured by noise.

Our experiments were designed to mitigate several possible sources of systematic noise: i) each of the 20 predicates occurred in the same sentence structures and was combined with the same complement clauses; ii) each of the 20 predicates was presented as an utterance or a proposition in the same context; iii) we compared ratings across different tasks%(recall that continuous slider measures and categorical selection measures yielded the same results)
; and iv) we collected judgments from a large number of participants, taking by-participant variability into consideration in the statistical analysis. Regarding projection, Exps.~1 yielded systematicity in gradient orderings of predicates across continuous (slider ratings) and categorical (selections) response measures. Exp.~1 of \citealt{degen-tonhauser-openmind} replicated Exp.~1a with a Spearman rank correlation of .991. The gradient orderings of predicates were also found in the meta-analyses of the existing datasets. Regarding entailment, Exps.~2 also yielded systematicity across continuous and categorical response measures, as well as across different kinds of data in the meta-analyses, and for different entailment diagnostics. This systematicity is hard to explain under a noise story. 
...
. \jt{don't know how to continue here} \jd{add the fuzzy category business here}
