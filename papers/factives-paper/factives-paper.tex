...
\jt{NEW:} Where do we go from here? There are several possible ways forward. A first one is to either maintain one of the definitions in (\ref{def}), but to use different diagnostics for presupposed or entailed content, or to revise the definition of factive predicates, and to develop and test novel diagnostics for their identification. Empirical investigations of such diagnostics constitute an important avenue of research for those who would posit a class of factive predicates. In the remainder of this section, we consider three additional ways forward. We limit our attention here to the projection gradience observed in Exps.~1 and in the meta-analyses of the existing datasets, that is, to the assumption that the CC of factive predicates is presupposed, which is common to both definitions in (\ref{def}). \jt{Limitation ok?} In section \ref{s41}, we discuss the possibility that a binary factivity category may have been obscured in our experiments by noise or performance factors. This discussion is couched in a more general consideration of the relationship between linguistic categories and gradient behavioral judgments. A second way forward was suggested to us in personal communication at conferences and by a reviewer: fitting alternative models, namely so-called mixture models, to the data to understand whether the observed gradience can be attributed to a binary factivity category. We briefly introduce mixture models in section \ref{s42}, and then motivate why we currently do not consider their application a principled way forward. Finally, a third way forward is to give up on the assumption that there is a coherent class of factive predicates (see also \citealt{karttunen2016} and \citealt{djaerv-thesis} for such moves). In section \ref{s43}, we discuss what this move would mean for projection analyses, one of the areas in which factivity has played a central role in theorizing. 

\subsection{On the relationship between linguistic categories and gradient behavioral judgments}\label{s41}

%While our results strongly suggest that projection and entailment do not identify a class of factive predicates, these results are contingent on how we operationalized projection and entailment, and how we measured them behaviorally. Our results are therefore compatible with the assumption that one of the two definitions in (\ref{def}) identifies a class of factive predicates and that the diagnostics we used in our experiments are the appropriate diagnostics to use if the experimental setup introduced noise into participants' response behavior that masked a binary factivity category. There are multiple ways in which such noise might have been introduced: i) our stimuli did not consist of the predicates alone, but of sentences in which the predicates were combined with particular subject noun phrases and complement clauses; ii) the sentences containing the predicates were used in an utterance (by a named speaker) or presented as a true proposition; iii) we used particular response tasks to collect ratings, providing participants with particular response options; and iv) human participants provided the ratings. Each of i) to iv) could have introduce systematic noise that, one could hypothesize, created gradience even though each predicate is either factive or not. 
...
%or presented as a true proposition
; iii) we used particular response tasks to collect ratings, providing participants with particular response options; and iv) human participants provided the ratings. Each of i) to iv) could have introduce systematic noise that, one could hypothesize, created gradience even though each predicate is either factive or not. 

Our experiments were designed to mitigate several possible sources of systematic noise: i) each of the 20 predicates occurred in the same sentence structures and was combined with the same complement clauses; ii) each of the 20 predicates was presented as an utterance 
%or a proposition 
in the same context; iii) we compared ratings across different tasks
; and iv) we collected judgments from a large number of participants, taking by-participant variability into consideration in the statistical analysis. Regarding projection, Exps.~1 yielded systematicity in gradient orderings of predicates across continuous (slider ratings) and categorical (selections) response measures. Exp.~1 of \citealt{degen-tonhauser-openmind} replicated Exp.~1a with a Spearman rank correlation of .991. The gradient orderings of predicates were also found in the meta-analyses of the existing datasets. 
%Regarding entailment, Exps.~2 also yielded systematicity across continuous and categorical response measures, as well as across different kinds of data in the meta-analyses, and for different entailment diagnostics. 
This systematicity is difficult to explain under a noise story. 
