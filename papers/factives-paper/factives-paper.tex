...
Exp.~2a investigated which of the CCs of the 20 clause-embedding predicates are entailed based on the inference diagnostic for entailment in (\ref{diag}a). Participants were presented with true statements of indicative matrix sentences with the 20 clause-embedding predicates and assessed whether the CC follows on a gradient scale.
...
\jd{JD Describe model and results:} In other words, the inference ratings to the CCs of {\em xx} and {\em xx} are indistinguishable from those of the entailing controls, whereas those of the remaining predicates, including {\em xxl} and {\em xx}, are significantly lower. 

\subsubsection{Discussion}

\jd{move up a lot of the discussion from what's currently sec 3.5, segue to categorical inference diagnostic}

It is possible that asking participants to assess whether the CC follows from a true statement of the indicative matrix sentence on a gradient response scale encouraged them to provide non-categorical answers and the finding is an artifact of the task. If so, the CCs of more clause-embedding predicates may be identified as entailed if participants were asked to assess whether the CC follows with a categorical response task. To assess this possibility, we conducted Exp.~2b, which was identical to Exp.~2a but employed a two-alternative forced choice task.
