...
These differences between the findings of our Exps.~2 and 3 stand in the way of conclusively identifying which predicates are factive. One issue is that the inference and the contradictoriness diagnostic deliver different results about which predicates have entailed CCs; given these differences, the two diagnostics do not appear to both diagnose entailment, contrary to assumption. Given how widely these diagnostics are used, a pressing question for future research is which diagnostic is better suited to diagnosing entailment.\footnote{We note that the entailed controls received very high ratings, under both diagnostics and under both the gradient and the categorical measures. This suggests that both diagnostics are able to diagnose entailed content. \citet[329]{demarneffe-etal2012} suggested that veridicality ratings are influenced by pragmatic reasoning. We agree with this possibility but consider it unlikely that pragmatic reasoning would only influence participants' ratings on the target stimuli, to the exclusion of the control stimuli. \jt{cross-diagnostic Spearman rank correlations:} .953 (gradient), .934 (binary); includes controls!}  

In this paper, we do not engage with this question but rather point to another issue that emerges from our findings. Recall from section \ref{s2} that the CCs of more predicates than previously assumed are projective, i.e., presupposed. A consequence of this empirical finding is that relatively more importance falls to entailment in identifying factive predicates, under definition (\ref{def}b): of all the predicates with projective, i.e., presupposed, CCs, those whose CC is entailed are factive and those whose CC is not entailed are not factive. For instance, for the 20 predicates we investigated, all of whose CCs are projective, i.e., presupposed, this had the unfortunate consequence that the set of factive predicates is, under the inference diagnostic, the heterogeneous set consisting of {\em prove, be right, see, discover, confirm} and possibly {\em know} or, under the contradictoriness diagnostic, the empty set. In other words, because so many more predicates have presupposed CCs, whichever predicate has an entailed CC also has a good chance of being factive. We conclude that considering entailment in the identification of factive predicates does not have the desired effect and, hence, that the definition of factive predicates in (\ref{def}b) fails.

Converging evidence again comes from the VerbVeridicality and MegaVeridicality datasets, which both include entailment ratings in addition to the projection ratings discussed in section \ref{s-converging1}. Consider first the VerbVeridicality dataset: in addition to projection ratings for the CCs of the negated variants of the 859 indicative matrix sentences, it also contains entailment ratings for the 859 indicative matrix sentences; an example is given in (\ref{vv-stim-proj2}). To diagnose entailment, three participants rated on a 5-point Likert scale whether the CC is true given the target sentence; one endpoint of the scale was labeled `definitely true' (coded as 2) and the other `definitely not true' (coded as -2). We follow \citet{ross-pavlick2019} here in refering to these ratings as `veridicality' ratings. On the standard definition of entailment, one would expect the three participants to rate entailed CCs as definitely true, i.e., for predicates with entailed CCs to have a mean veridicality rating of 2.\footnote{Relaxing the linking function does not improve the situation: the three predicates with a mean veridicality rating of at least 1.7 (one of the three raters gave a `1') include the non-veridical predicate {\em explain} and the 25 predicates with a mean veridicality rating of at least 1.3 (two of the three raters gave a `1') additionally include the non-veridical predicates {\em announce, mention, add} and {\em note}.}
...
\begin{exe}
\exi{(i)} Rambling to reporters in the Oval Office, Trump bitched that the Democratic party clearly hates all the Jews because of all the support reps Rashida Tlaib and Ilhan Omar received after he strong armed the Israelis into barring them from visiting the country as members of Congress.\footnote{\url{https://www.wonkette.com/08-21-2019}}
\end{exe}
