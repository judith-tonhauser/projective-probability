...
The possibility of categorizing clause-embedding predicates by the projectivity of the CC is further called into question by the fact that the CC of all of the 20 predicates investigated is projective, albeit to varying degrees, compared to the non-projective main clause controls. This was established by fitting a Bayesian mixed effects Beta regression model  with weakly informative priors using the \verb|brms| \citep{buerkner2017}  package in R \citep{R}.\footnote{We conducted a Beta regression instead of a linear regression because linear regression assumes normally-distributed residuals, an assumption that was violated by this dataset. Beta regression is useful because it allows for modeling an arbitrarily distributed outcome variable in the $[$0,1$]$ interval. Because the outcome variable contained the values 0 and 1, which Beta regression is undefined for, we applied a common transformation to ratings before the main analysis that rescales values to fall between 0 and 1  \citep{smithson2006}.} The model predicted the certainty ratings from a fixed effect of predicate (with treatment coding and `main clause' as  reference level) and included the maximal random effects structure justified by the design: random by-participant and by-content intercepts and random by-participant and by-content slopes for the fixed effect of predicate (capturing random variability in overall projectivity by content and participant as well as random variability in the effect of predicate, which may vary by participant and content). In contrast to linear regression, which assumes the outcome variable is normally distributed and only models means, Beta regression models both mean and precision (a measure of dispersion) of the outcome distribution, and thus returns a mean and precision coefficient for each fixed effect contrast.  For model details, see Appendix \ref{modeldetails}.

Mean certainty ratings for all predicates were higher than for the main clause controls, as established in two ways. First, the  estimated mean $\mu$ for each predicate was higher than that of the main clause controls (i.e., the 95\% credible intervals for the mean estimate did not contain 0 for any predicate). Second, with the exception of the predicates \emph{pretend},  \emph{think}, \emph{suggest}, and \emph{say}, the estimated value of the precision parameter $\phi$ for each predicate was lower than that of the main clause controls (i.e., the 95\% credible intervals for the precision estimate did not contain 0 for any predicate), indicating greater variance in certainty ratings than the main clause controls for all predicates except the above mentioned.  See Appendix \ref{modeldetails}   for the full table of model estimates.  {\bf DESCRIBE FINDINGS FOR LEAST PROJECTIVE PREDICATES?} \jd{not sure what you have in mind here}
...
In the following, we provide details on the data analysis conducted for Exps.~1a, 2a, and 3a. We begin by motivating the use of Beta regression rather than linear regression. We then provide a brief primer on how to interpret Bayesian mixed effects Beta regression models. We then report the model output in detail for each experiment in the main body of the text. 
...
We allowed both $\mu$ and $\phi$ to vary as a function of predicate, with reference level set to main clause control (Exp.~1a), entailing control (Exp.~2a), and contradictory control (Exp.~3a), respectively. We also allowed random intercept adjustments to each parameter by subject and by item, where item was defined as a unique combination of predicate and lexical content. Four chains converged after 2000 iterations each (warmup = 1000, \(\hat{R}=1\) for all estimated parameters) with a target acceptance rate of .95 and a maximum treedepth of 15.
