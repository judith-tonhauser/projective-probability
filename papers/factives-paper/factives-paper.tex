...
Empirical observations like these mean that research on projective content must assess which classes of projective content there are and formal analyses of presuppositions must reassess which classes of projective content they apply to. Both of these tasks are outside the scope of this paper (but see, e.g., \citealt{kadmon01,potts05,abrusan2011,abrusan2016,brst-lang11,tbd-variability,tonhauser-etal-sub23} and references therein). In this paper, we pursue the question of which predicates are factive by following what is, to this day, the standard practice in the field: projection is assumed to distinguish presupposed CCs from non-presupposed ones and, therefore, projection of the CC is a sufficient condition for a predicate to be factive, on the definition in (\ref{def}a), and a necessary condition, on the definition in (\ref{def}b).\footnote{Only the second assumption is made by \citet{anand-hacquard2014}, who assume the definition in (\ref{def}b). They suggest (pp.74f.) that even though the CC of predicates like {\em acknowledge, admit} and {\em confirm} projects, the CC is not presupposed, i.e., projection of the CC is not due to the speaker presupposing the truth of the CC. They do not, however, offer a diagnostic for distinguishing projective CCs that are presupposed from projective CCs that are not presupposed. Ultimately, their argument against classifying such predicates as factive rests on the observation that their CC is not entailed; see their example (15). It is an interesting question for future research how presupposed projective CCs can be distinguished from non-presupposed projective CCs.} The goal of this paper, then, is to investigate which predicates are factive: on the definition in (\ref{def}a), projection of the CC is a sufficient condition, whereas it is a necessary one on the definition in (\ref{def}b).
