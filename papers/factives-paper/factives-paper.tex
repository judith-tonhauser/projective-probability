...
Where do we go from here? We see two broad ways forward. The first one is to keep looking for a class of factive predicates. Note that while our results strongly suggest that projection and entailment do not identify a class of factive predicates, that result is entirely contingent on the way we have measured projection and entailment behaviorally. Thus, our results do not conclusively show that there is no class of factive predicates. In section \ref{s41}, we discuss what evidence for a class of factive predicates might look like. The second way forward is to give up on the assumption that there is a coherent class of factive predicates (see also \citealt{karttunen2016} and \citealt{djaerv-thesis} for such moves). In section \ref{s42}, we discuss what this move would mean for one of the areas in which factivity has played a central role in theorizing, namely projection analyses. 

\subsection{Continuing the search for a class of factive predicates}\label{s41}

There are different paths on which one can continue the search for a class of factive predicates. One path would be to argue that factive predicates should be defined differently than in the two definitions in (\ref{def}), and that, consequently, other diagnostics should be used to identify factive predicates. Another option would be to maintain one of the definitions in (\ref{def}), but to use different diagnostics for presupposed or entailed content. Empirical investigations with such diagnostics would be an important step for those who want to provide empirical evidence for a class of factive predicates.

It is also possible, however, that the fact that our experiments did not provide empirical support for a categorical factivity distinction has nothing to do with the definitions investigated or the diagnostics used. Our results are in fact compatible with the assumption that one of the two definitions in (\ref{def}) identifies a class of factive predicates and that the diagnostics we used in our experiments are the appropriate diagnostics to use. This is a possibility if we consider that our experiments may have introduced noise into the response behavior that is independent of the predicates, and that masked a binary factivity category. There are multiple ways in which such noise might have been introduced: i) our stimuli did not consist of the predicates alone, but of sentences in which the predicates were combined with particular subject noun phrases and complement clauses; ii) the sentences containing the predicates were used in an utterance (by a named speaker) or presented as a true proposition; iii) we used particular response tasks to collect ratings, providing participants with particular response options; and iv) human participants provided the ratings. Each of i) to iv) could have introduce systematic noise that, one could hypothesize, created gradience even though each predicate is either factive or not, thereby obscuring a binary factivity category. In other words, we cannot conclude from the fact that our experiments did not provide empirical support for a categorical factivity distinction that there is no such distinction because the distinction may exist but be obscured by noise.

Our experiments were designed to mitigate several possible sources of systematic noise: i) each of the 20 predicates occurred in the same sentence structures and was combined with the same complement clauses; ii) each of the 20 predicates was presented as an utterance or a proposition in the same context; iii) we compared ratings across different tasks (recall that gradient and binary tasks gave same results); and iv) we collected judgments from a lot of participants, taking by-participant variation into consideration in the statistical analysis. Regarding projection, Exps.~1 found systematicity in gradient orderings of predicates across gradient and categorical response tasks. Exp.~1 of \citealt{degen-tonhauser-openmind} replicated Exp.~1a with a Spearman rank correlation of .991. The gradient orderings of predicates were also found in the meta-analyses of the existing datasets. Regarding entailment, we again found systematicity across gradient and categorical response tasks, as well as across different kinds of data in the meta-analyses, and for different diagnostics for entailment. This systematicity is what makes us think that it is unlikely that there is a binary factivity category that is covered up by systematic noise. 

But it is still a possibility. \jt{don't know how to continue here}
...
\item Things one can do: mixture models.
