...
\jt{revise introduction: clarify the relation between presupposition and projection (reviewer A); clarify how projection and entailment are diagnosed and thereby distinguished (AE); mention that different authors/analyses assume the two definitions in (10) -- they are not yet analyses; To be very clear: what is given in (\ref{def}) are definitions of factive predicates, that is, statements intended to distinguish a particular class of predicates based on empirical properties. They are not formal analyses of the projection of the CC of factive predicates, as also evidenced by the fact that different formal analyses assume the two definitions.
\\
clarify that not everything that projects is a presupposition and that presuppositions are heterogeneous
\\
why do the definitions still use ``presupposed''? how can we evaluate presuppositionality using projection?
\\
make sure readers don't come away from KK1970 discussion with the idea that we propose a third class of predicates that is coherent
\\
In this connection, we have also clarified that the assumption that the CC is presupposed, which underlies both definitions in (10), is made regardless of presupposed content is formally analyzed and whether presuppositions are conceived of as semantic or pragmatic.}
