...
As illustrated above, \citealt{kiparsky-kiparsky70} characterized presupposed content as content that is not part of the main assertion and diagnosed it using the projection diagnostic. This view of presuppositions has been standard for a long time: in \citealt{beaver-geurts-sep}, for instance, presupposed content is characterized as content that is ``taken for granted, rather than being part of the main propositional content of a speech act'' and ``[p]rojection from embeddings, especially negation, is standardly used as a diagnostic for presupposition''. But the picture is more complex because presuppositions are not the only backgrounded content that projects: conventional implicatures, like those contributed by appositives, are backgrounded, projective content, too (see, e.g., \citealt{ccmg90,potts05,amaral-etal07}). Although projection alone does not suffice to categorize a CC as a presupposition, presuppositions and conventional implicatures differ in that conventional implicatures are unacceptable in a context in which their content is already part of the common ground; this is \citepos{potts05} antibackgrounding requirement. Consider (\ref{cips}), which provides the content that Lance Armstrong survived cancer. The continuation in (\ref{cips}a) is unacceptable because this content is conventionally implicated by the appositive {\em a cancer survivor}. The continuation in (\ref{cips}b), on the other hand, is acceptable: here, the content is presupposed, as the CC of {\em know}. 

\begin{exe}
\ex\label{cips} \citealt[34]{potts05} \\ Lance Armstrong survived cancer.
\begin{xlist}
...
\end{xlist}
\end{exe}
Like the CC of {\em know}, the CCs of clause-embedding predicates more generally are acceptable even if their content is already part of the common ground: for instance, the example in (\ref{cips}b) is acceptable when {\em know} is replaced with {\em say, believe, take into account} or {\em confirm}. Therefore, even though not every projective content is a presupposition, we can continue to assume that a projective CC is a presupposition, as is standard practice in the field.\footnote{One exception are \citet{anand-hacquard2014}, who suggest (pp.74f.) that even though the CC of predicates like {\em acknowledge, admit} and {\em confirm} projects, the CC is not presupposed, i.e., projection of the CC is not due to the speaker presupposing the truth of the CC. They do not, however, offer a diagnostic for distinguishing projective CCs that are presupposed from projective CCs that are not presupposed. Ultimately, their argument against classifying such predicates as factive rests on the observation that their CC is not entailed; see their example (15).}  
